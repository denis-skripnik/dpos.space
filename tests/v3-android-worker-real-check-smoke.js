...
assert(mainActivity.includes('.put("autoUpvoterFeed", store.exportAutoUpvoterFeed())'), 'native worker status exposes persisted auto-upvoter feed to WebView');
assert(appSource.includes('runtime.nativeWorkerStatus') && appSource.includes('loadAutoUpvoterBatterySummary(collectSettings())') && appSource.includes('Последняя native-лента пока пуста'), 'auto-upvoter page loads battery/status for running native worker and does not show stale tab-only placeholder');
