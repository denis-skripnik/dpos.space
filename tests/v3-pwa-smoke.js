...
assert(indexSource.indexOf('v3/js/pwa.js') < indexSource.indexOf('v3/js/app.native-notification-settings.js'), 'PWA helper loads before app runtime');
...
assert(swSource.includes("'/v3/js/app.native-notification-settings.js'"), 'service worker caches physically versioned app runtime');
assert(indexSource.includes('v3/js/app.native-notification-settings.js') && indexSource.includes('v3/css/style.css?v='), 'index uses a physically versioned app runtime path plus versioned CSS to bypass stale browser/WebView/CDN cache');
