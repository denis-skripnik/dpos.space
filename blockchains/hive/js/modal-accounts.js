...
posting = sjcl.encrypt('dpos.space_hive_' + login + '_postingKey', posting_key);
}
}
let active = '';
if (active_key) {
...
if (active_public_keys.includes(public_wif)) {
	active = sjcl.encrypt('dpos.space_hive_' + login + '_activeKey', active_key);
}
}
...
if (!localStorage.getItem('hive_users')|| localStorage.getItem('hive_users').indexOf(login) === -1) {
if (posting && active) {
