...
var vote_power = document.getElementById('vp').value;
    var vote_weight = document.getElementById('vote_weight').value
	  $("#let1").load("/blockchains/hive/apps/calc/ajax.php","type=result_power&sp=" + hive_power + "&charge=" + vote_power + "&vote_weight=" + vote_weight, function(result, status) {
          if (status === 'error') {
              console.log(result);
          }
      });
...
var sp_tec = document.getElementById('sp_tec').value
	  $("#let2").load("/blockchains/hive/apps/calc/ajax.php","type=result_vests&sp-tec=" + sp_tec, function(result, status) {
          if (status === 'error') {
              console.log(result);
          }
      });
