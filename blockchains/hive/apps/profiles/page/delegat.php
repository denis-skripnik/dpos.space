    ...
    if ($witness_count > 0) {
        $result['content'] .= "<ul>";
            ...
            for ($witness_votes_num = 0; $witness_votes_num <= $witness_votes_count-1; $witness_votes_num++) {
                ...
                $result['content'] .= '<li><a href="'.$siteUrl.'hive/profiles/'.$witness_vote.'" target="_blank">'.$witness_vote.'</a></li>';
            }
                $result['content'] .= "</ul>";
    } else {
        $iHProxy = '';
        if ($datas['proxy'] != '') {
            $iHProxy = ', но установил в качестве прокси голосования за делегатов <a href="'.$siteUrl.'hive/profiles/'.$datas['proxy'].'" target="_blank">@'.$datas['proxy'].'</a>';
        }
...
$timestamp = date('j', $timestamp2).' '.$month[$month2].' '.date('Y г. H:i:s', $timestamp2);
		...
		if ($op[0] == 'account_witness_vote') {
            ...
            $result['content'] .= '<tr>
<td>'.$timestamp.'</td>
<td><a href="'.$siteUrl.'hive/profiles/'.$vote_account.'" target="_blank">'.$vote_account.'</a> '.$true_vote.' делегата <a href="'.$siteUrl.'hive/profiles/'.$witness_account.'" target="_blank">'.$witness_account.'</a></td>
</tr>';
