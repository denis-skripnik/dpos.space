...
if ($op[0] == 'claim') $memo = 'Получение своих начислений на HP.';
...
$result['content'] .= '<tr>
<td>'.$timestamp.'</td>
        <td><a href="'.$site_url.'hive/profiles/'.$creator.'" target="_blank">'.$creator.'</a></td>
<td></td>
<td>'.$balance.'</td>
<td>'.$memo.'</td>
</tr>';
...
$initiator = $op[1]['initiator'];
    ...
    $result['content'] .= '<tr>
    <td>'.$timestamp.'</td>
            <td><a href="'.$site_url.'hive/profiles/'.$initiator.'" target="_blank">'.$initiator.'</a></td>
    <td><a href="'.$site_url.'hive/profiles/'.$receiver.'" target="_blank">'.$receiver.'</a></td>
    <td></td>
    <td>'.$memo.'</td>
    </tr>';
                    if ($rowCount === TRX_LIMIT) {
