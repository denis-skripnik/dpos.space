...
const ops = {
    ...
    set_withdraw_vesting_route: 'Направление вывода в HP',
    ...
    delegate_vesting_shares: 'Делегирование HP',
    account_create_with_delegation: 'Создание аккаунта с делегированием HP',
    ...
    interest: 'Выплата процента прибыли по HBD',
    fill_vesting_withdraw: 'Завершение вывода из HP',
...
let props = await hive.api.getDynamicGlobalPropertiesAsync();
let tvfs = parseFloat(props['total_vesting_fund_hive']);
let tvsh = parseFloat(props['total_vesting_shares']);

let hive_per_vests = 1000000 * tvfs / tvsh;

   let oldHistory = await hive.api.getAccountHistoryAsync(user, from, limit);
...
for (let field in opData) {
    ...
    if (typeof fieldValue === 'string' && fieldValue !== '' && userFields.indexOf(field) > -1) fieldValue = `<a href="/hive/profiles/${fieldValue}" target="_blank">@${fieldValue}</a>`;
    if (typeof fieldValue === 'string' && fieldValue.indexOf('VESTS') > -1 || typeof fieldValue === 'string' && fieldValue.indexOf('GESTS') > -1)    fieldValue = Math.round(parseFloat(fieldValue) / 1000000 * hive_per_vests, 3);
