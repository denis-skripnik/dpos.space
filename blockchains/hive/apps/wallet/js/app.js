...
function cancelDelegatedVestingShares(delegatee) {
  hive.broadcast.delegateVestingShares(active_key, hive_login, delegatee, '0.000000 VESTS', function(err, result) {
    if (!err) {
...
body_delegated_vesting_shares = '<tr id="delegated_vesting_shares_' + item.delegatee + '"><td><a href="/hive/profiles/' + item.delegatee + '" target="_blank">@' + item.delegatee + '</a></td><td>' + vesting_shares_amount + '</td><td>' +  min_delegation_datetime + '</td><td><input type="button" onclick="cancelDelegatedVestingShares(`' + item.delegatee + '`);" value="Отменить делегирование"></td></tr>';
