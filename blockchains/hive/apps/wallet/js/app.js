...
body_delegated_vesting_shares = '<tr id="delegated_vesting_shares_' + item.delegatee + '"><td><a href="/hive/profiles/' + item.delegatee + '" target="_blank">@' + item.delegatee + '</a></td><td>' + vesting_shares_amount + '</td><td>' +  min_delegation_datetime + '</td><td><input type="button" id="cancel_delegated_vesting_shares_' + item.delegatee + '" value="Отменить делегирование"></td></tr>';
...
function prepareContent(text) {
  ...
  }).replace(/ (@[^< \.,]+)/gi, user => ` <a href="/hive/profiles/${user.trim().slice(1)}">${user.trim()}</a>`)
 }
...
<td><a href="/hive/profiles/' + from + '" target="_blank">@' + from + '</a></td>\
<td><a href="/hive/profiles/' + to + '" target="_blank">@' + to + '</a></td>\
<td>' + amount + '</td>\
<td>' + memo + '</td>\
</tr>');
...
<td>' + from + '</td>\
<td><a href="/hive/profiles/' + witness + '" target="_blank">@' + witness + '</a></td>\
<td>' + shares + '</td>\
<td>' + memo + '</td>\
</tr>');
  }
