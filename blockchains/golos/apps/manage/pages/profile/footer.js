...
if(typeof metadata.profile.name !== 'undefined'){
            ...
            if(typeof metadata.profile.select_tags !== 'undefined'){
                            $('input[name=Interests]').val(metadata.profile.select_tags.join(','));
            }
...
if ($('.profile-update select[name=gender]').val()) {
    metadata.profile.gender=$('.profile-update select[name=gender]').val().trim();
            ...
            i2 = unique(i2);
            metadata.profile.select_tags = i2;
           ...
           golos.broadcast.accountMetadata(posting_key,golos_login,json_metadata,function(err, result){
                if(!err){
