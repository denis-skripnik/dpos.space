...
var ltmp_arr={
	...
	index_social_capital_caption:'СГ',
	index_balance_caption:'Баланс (GOLOS)',
...
function manage_access_save(account,owner_key,el){
	el.find('.manage-access-preload-error').html('');
	el.find('.manage-access-preload-success').html('');
	if(!golos.auth.isWif(owner_key)){
		el.find('.manage-access-save-error').html(ltmp_arr.default_invalid_owner_key);
		return;
	}
	let owner_weight_threshold=parseInt(el.find('input[name=owner-weight-threshold]').val());
	let active_weight_threshold=parseInt(el.find('input[name=active-weight-threshold]').val());
	let posting_weight_threshold=parseInt(el.find('input[name=posting-weight-threshold]').val());
	if((parseInt(owner_weight_threshold)<=0)||(isNaN(parseInt(owner_weight_threshold)))){
		el.find('.manage-access-save-error').html(ltmp_arr.access_invalid_owner_weight_threshold);
		return;
	}
	...
	if((parseInt(posting_weight_threshold)<=0)||(isNaN(parseInt(posting_weight_threshold)))){
		el.find('.manage-access-save-error').html(ltmp_arr.access_invalid_posting_weight_threshold);
		return;
	}

	let owner_weight_sum=0;
	let active_weight_sum=0;
	let posting_weight_sum=0;

	let owner_account_auths=[];
	let active_account_auths=[];
	let posting_account_auths=[];

	let owner_key_auths=[];
	let active_key_auths=[];
	let posting_key_auths=[];

	let to_save=[];

	el.find('.account-keys .owner-accounts .account-auths').each(function(i,auth_el){
		owner_account_auths.push([$(auth_el).attr('data-account1'),parseInt($(auth_el).attr('data-weight'))]);
		owner_weight_sum+=parseInt($(auth_el).attr('data-weight'));
	});
	...
	el.find('.account-keys .posting-accounts .account-auths').each(function(i,auth_el){
		posting_account_auths.push([$(auth_el).attr('data-account3'),parseInt($(auth_el).attr('data-weight'))]);
		posting_weight_sum+=parseInt($(auth_el).attr('data-weight'));
	});

	el.find('.account-keys .owner-keys .key-auths').each(function(i,auth_el){
		owner_key_auths.push([$(auth_el).attr('data-key'),parseInt($(auth_el).attr('data-weight'))]);
		if(typeof $(auth_el).attr('data-private-key') !== 'undefined'){
			to_save.push(['owner',$(auth_el).attr('data-private-key')]);
		}
		owner_weight_sum+=parseInt($(auth_el).attr('data-weight'));
	});
	...
	el.find('.account-keys .posting-keys .key-auths').each(function(i,auth_el){
		posting_key_auths.push([$(auth_el).attr('data-key'),parseInt($(auth_el).attr('data-weight'))]);
		if(typeof $(auth_el).attr('data-private-key') !== 'undefined'){
			to_save.push(['posting',$(auth_el).attr('data-private-key')]);
		}
		posting_weight_sum+=parseInt($(auth_el).attr('data-weight'));
	});

	if(owner_weight_sum<owner_weight_threshold){
		el.find('.manage-access-save-error').html(ltmp_arr.access_need_owner_weight);
		return;
	}
	...
	if(posting_weight_sum<posting_weight_threshold){
		el.find('.manage-access-save-error').html(ltmp_arr.access_need_posting_weight);
		return;
	}
	...
	let posting = {
		'weight_threshold': posting_weight_threshold,
		'account_auths': posting_account_auths,
		'key_auths': posting_key_auths
	};
...
function manage_access_preload(account,el){
	...
	el.find('.account-keys .owner-accounts .account-auths').remove();
	el.find('.account-keys .owner-keys .key-auths').remove();
	el.find('.account-keys .active-accounts .account-auths').remove();
	el.find('.account-keys .active-keys .key-auths').remove();
	el.find('.account-keys .posting-accounts .account-auths').remove();
	el.find('.account-keys .posting-keys .key-auths').remove();
	...
	el.find('.account-keys input[name=manage-access-owner-key]').val('');
	el.find('.account-keys input[name=manage-access-owner-key]').attr('data-account','');
	...
	golos.api.getAccounts([account],function(err,response){
		if(typeof response[0] !== 'undefined'){
			...
			el.find('.account-keys .owner-accounts .none-auths').css('display','none');
			el.find('.account-keys .owner-keys .none-auths').css('display','none');
			...
			el.find('.account-keys .posting-accounts .none-auths').css('display','none');
			el.find('.account-keys .posting-keys .none-auths').css('display','none');
			...
			el.find('.account-keys input[name=manage-access-owner-key]').attr('data-account',account);

			el.find('.account-keys input[name=owner-weight-threshold]').val(response[0].owner.weight_threshold);
			el.find('.account-keys input[name=active-weight-threshold]').val(response[0].active.weight_threshold);
			el.find('.account-keys input[name=posting-weight-threshold]').val(response[0].posting.weight_threshold);
			...
			counter=0;
			for(i in response[0].active.account_auths){
				let new_el='<div class="account-auths" data-account2="'+response[0].active.account_auths[i][0]+'" data-weight="'+response[0].active.account_auths[i][1]+'">';
				new_el+=response[0].active.account_auths[i][0];
				new_el+='<div class="adaptive-show-block"></div>';
				new_el+= '<span class="weight-inline">'+ltmp(ltmp_arr.access_weight_caption,{weight:response[0].active.account_auths[i][1]})+'</span>';
				new_el+='<a class="red-button-inline delete-auth-action" onclick=" el = $(`div[data-account2='+response[0].active.account_auths[i][0]+']`); deleteauthaction(el);">'+ltmp_arr.access_remove_caption+'</a>';
				new_el+='</div>';
			...
			counter=0;
			for(i in response[0].active.key_auths){
				let new_el='<div class="key-auths" data-key="'+response[0].active.key_auths[i][0]+'" data-weight="'+response[0].active.key_auths[i][1]+'">';
				new_el+=response[0].active.key_auths[i][0];
				new_el+='<div class="adaptive-show-block"></div>';
				new_el+=' <span class="weight-inline">'+ltmp(ltmp_arr.access_weight_caption,{weight:response[0].active.key_auths[i][1]})+'</span>';
				new_el+='<a class="red-button-inline delete-auth-action" onclick="el = $(`div[data-key='+response[0].active.key_auths[i][0]+']`); deleteauthaction(el);">'+ltmp_arr.access_remove_caption+'</a>';
				new_el+='</div>';
			...
			counter=0;
			for(i in response[0].posting.account_auths){
				let new_el='<div class="account-auths" data-account3="'+response[0].posting.account_auths[i][0]+'" data-weight="'+response[0].posting.account_auths[i][1]+'">';
				new_el+=response[0].posting.account_auths[i][0];
				new_el+='<div class="adaptive-show-block"></div>';
				new_el+=' <span class="weight-inline">'+ltmp(ltmp_arr.access_weight_caption,{weight:response[0].posting.account_auths[i][1]})+'</span>';
				new_el+='<a class="red-button-inline delete-auth-action" onclick=" el = $(`div[data-account3='+response[0].posting.account_auths[i][0]+']`); deleteauthaction(el);">'+ltmp_arr.access_remove_caption+'</a>';
				new_el+='</div>';
				el.find('.account-keys .posting-accounts').append(new_el);
				++counter;
			}
			if(0==counter){
				el.find('.account-keys .posting-accounts .none-auths').css('display','block');
			}
			counter=0;
			for(i in response[0].posting.key_auths){
				let new_el='<div class="key-auths" data-key="'+response[0].posting.key_auths[i][0]+'" data-weight="'+response[0].posting.key_auths[i][1]+'">';
				new_el+=response[0].posting.key_auths[i][0];
				new_el+='<div class="adaptive-show-block"></div>';
				new_el+=' <span class="weight-inline">'+ltmp(ltmp_arr.access_weight_caption,{weight:response[0].posting.key_auths[i][1]})+'</span>';
				new_el+='<a class="red-button-inline delete-auth-action" onclick=" el = $(`div[data-key='+response[0].posting.key_auths[i][0]+']`); deleteauthaction(el);">'+ltmp_arr.access_remove_caption+'</a>';
				new_el+='</div>';
				el.find('.account-keys .posting-keys').append(new_el);
				++counter;
			}
			if(0==counter){
				el.find('.account-keys .posting-keys .none-auths').css('display','block');
			}
		}
		else{
...
var addkeyauthsaction = function(el){
		...
		if(!golos.auth.isPubkey(public_key)){
		...
		if(''!=private_key){
			if(!golos.auth.isWif(private_key)){
			...
			if(golos.auth.wifToPublic(private_key)!=public_key){
				$('.page-manage-access .account-keys-'+type+' .add-key-auths input[name=private-key]').addClass('red');
				$('.page-manage-access .account-keys-'+type+' .add-key-auths input[name=public-key]').addClass('red');
				$('.page-manage-access .account-keys-'+type+' .add-key-auths input[name=private-key]').focus();
				return;
