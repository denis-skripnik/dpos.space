...
function proxyVote(field_name) {
    ...
    golos.broadcast.accountWitnessProxy(active_key, golos_login, proxy_login, function(err, result) {
if (!err) {
...
function oneWitnessVote() {
    ...
    golos.broadcast.accountWitnessVote(active_key, golos_login, witness_login, true, function(err, result) {
if (!err) {
...
return false;
    ...
    golos.broadcast.send({extensions: [], operations}, [active_key], function(error, res) {
if (!error) {
