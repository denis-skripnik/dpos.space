    ...
    if ($witness_count > 0) {
        $result['content'] .= "<ul>";
            ...
            for ($witness_votes_num = 0; $witness_votes_num <= $witness_votes_count-1; $witness_votes_num++) {
                ...
                $result['content'] .= '<li><a href="'.$siteUrl.'golos/profiles/'.$witness_vote.'" target="_blank">'.$witness_vote.'</a></li>';
            }
                $result['content'] .= "</ul>";
    } else {
        $isProxy = '';
        if ($datas['proxy'] != '') {
            $isProxy = ', но установил в качестве прокси голосования за делегатов <a href="'.$siteUrl.'golos/profiles/'.$datas['proxy'].'" target="_blank">@'.$datas['proxy'].'</a>';
        }
...
$no_delegate_votes = '';
    ...
    $history_result = getAccountHistoryChunk($user, $startWith, ['select_ops' => ["account_witness_vote"]]);
...
$timestamp = '<a href="'.$siteUrl.'golos/explorer/tx/'.$history_datas[1]['trx_id'].'" target="_blank">'.$timestamp.'</a>';
		...
		if ($op[0] == 'account_witness_vote') {
            ...
            $result['content'] .= '<tr>
<td>'.$timestamp.'</td>
<td><a href="'.$siteUrl.'golos/profiles/'.$vote_account.'" target="_blank">'.$vote_account.'</a> '.$true_vote.' делегата <a href="'.$siteUrl.'golos/profiles/'.$witness_account.'" target="_blank">'.$witness_account.'</a></td>
</tr>';
