...
$res = getAccountHistoryChunk($user, $startWith, ['select_ops' => ['comment_mention']]);
...
$mass = $res['result'];
    ...
    krsort($mass);
            ...
            $result['content'] = '<div id="ajax_content"><h2>Упоминания пользователя '.$user.'</h2>
    <table id="rewards-ol">
            <tr><th>Дата и время получения</th>
            <th>Кого упоминают</th>
        ...
        $timestamp = '<a href="'.$site_url.'golos/explorer/tx/'.$datas[1]['trx_id'].'" target="_blank">'.$timestamp.'</a>';
                $op1 = $op[1];
                if ($op[0] == 'comment_mention') {
                    $rowCount++;
                    $author = $op[1]['author'] ?? "";
                    $permlink = $op[1]['permlink'] ?? "";
                    $mentioned = $op[1]['mentioned'] ?? "";
                    $result['content'] .= '<tr><td>' . $timestamp . '</td>
<td><a href="'.$conf['siteUrl'].'golos/profiles/'.$mentioned.'" target="_blank">'.$mentioned.'</a></td>
