...
<th>Комиссия</th></tr>';
        ...
        $timestamp = '<a href="'.$site_url.'golos/explorer/tx/'.$datas[1]['trx_id'].'" target="_blank">'.$timestamp.'</a>';
                ...
                if ($op[0] == 'fill_order') {
                    ...
                    $fee_amount = $op[1]['current_trade_fee'];    
                    $result['content'] .= '<tr><td>' . $timestamp . '</td>
    <td><a href="'.$site_url.'golos/profiles/'.$seller.'" target="_blank">'.$seller.'</a></td>
    <td><a href="'.$site_url.'golos/profiles/'.$buyer.'" target="_blank">'.$buyer.'</a></td>
        <td>'.$sell_amount.'</td>
        <td>'.$buy_amount.'</td>
        <td>'.$fee_amount.'</td></tr>';
                    }
