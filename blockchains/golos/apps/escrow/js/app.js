...
function gup( name ) {
	...
	if (results == null) {
		results = localStorage.getItem('golos_escrow_' + name);
		if(results) {
			return results;
		} else {
			return '';
		...
		if(name == 'l' || name == 'blockchain') {
			localStorage.setItem('golos_escrow_' + name, results[1]);
		}
		return results[1];
	}
}
...
var BLOCKCHAINS = {
	golos: {
		...
		sbd_ticker: 'GBG',
		url: 'golos.id',
		agents_list_post_author: 'denis-skripnik',
		agents_list_post_permlink: 'ru-khocheshx-statx-garantom-budx-im',
	...
	matches = transaction.id.match(/^([a-z0-9\.\-]+)-([\d]+)-([golos|golos]+)$/),
...
function showTab() {
	$('#header ul li').show();
	$('#currentBlockchain').text(BLOCKCHAIN.golos_ticker);
	if(transaction.id) {
		$('#tabCP').click();
...
function loadTransaction() {
	...
	golos.api.getEscrow(
			...
			function(err, response) {
				if(!err && !response) {
					...
					if(transaction.steem_balance == '0.000 ' + BLOCKCHAIN.golos_ticker) {
					...
					$('span.transactionId').html('<a href="/golos/escrow/?id=' + transaction.id + '">' + transaction.id + '</a>');
	...
	transaction.submitApprove = function(approve, callback) {
		golos.broadcast.escrowApprove(
				active_key,
				transaction.from,
				transaction.to,
				transaction.agent,
	...
	transaction.submitRelease = function(callback) {
		golos.broadcast.escrowRelease(
				active_key,
				transaction.from,
				transaction.to,
				transaction.agent,
				golos_login,
				golos_login == transaction.from ? transaction.to : transaction.from,
				transaction.escrow_id,
	...
	transaction.submitExpired = function(callback) {
		golos.broadcast.escrowRelease(
				active_key,
				transaction.from,
				transaction.to,
				transaction.agent,
				golos_login,
				golos_login == transaction.from ? transaction.from : transaction.to,
				transaction.escrow_id,
				transaction.sbd_balance,
				transaction.steem_balance,
				function(err, response) {
	...
	transaction.submitDispute = function(callback) {
		golos.broadcast.escrowDispute(
				active_key,
				transaction.from,
				transaction.to,
				transaction.agent,
	...
	transaction.submitEscrow = function(reciever, callback) {
		golos.broadcast.escrowRelease(
				active_key,
				transaction.from,
				transaction.to,
...
function loadBoard() {
	golos.api.getContentReplies(BLOCKCHAIN.board_post_author, BLOCKCHAIN.board_post_permlink, function(err, result) {
		var orders = [],
			wrap = $('#boardBody'),
			messagesCount = 0,
			hrSkip = true;
...
$(function() {
	...
	$('a.inputToggle').click(function() {
		if($(this).text() == BLOCKCHAIN.golos_ticker) {
			$('.inputToggle').text(BLOCKCHAIN.sbd_ticker);
		} else {
			$('.inputToggle').text(BLOCKCHAIN.golos_ticker);
		}
		return false;
	});
		...
		switch($(this).attr('id')) {
			...
			case 'tabCP':
				if (!transaction.id) {
					...
					window.location.href = '/golos/escrow/?id=' + transaction.id;
				} else {
					history.pushState('', LNG.byElement.title, '/golos/escrow/?id=' + transaction.id);
				}
				break;
			case 'tabBoard':
				history.pushState('', LNG.byId.tabBoard, '/golos/escrow/?tab=board');
				break;
		}
	...
	$('form#formSend').submit(function() {
		...
		golos_amount = '0.000 ' + BLOCKCHAIN.golos_ticker,
		terms = {
		...
		if($('#aSendAmountUnit').text() == BLOCKCHAIN.golos_ticker) {
			golos_amount = parseFloat($('#inputSendAmount').val().replace(',','.')).toFixed(3) + ' ' + BLOCKCHAIN.golos_ticker;
		...
		golos.api.getDynamicGlobalProperties(function(err, response) {
			...
			//return false;
			golos.broadcast.escrowTransfer(
				wif, // active key
				from,
				to, // to
				agent, // escrow nick
				...
				golos_amount, // amount golos/golos
				fee, // fee
				...
				function(err, response) {
					if(!err && response.ref_block_num) {
						...
						$('#formSend').slideUp();
						$('#step2').slideDown();
						$('.sentLink').html('<a href="/golos/escrow/?id=' + from + '-' + escrow_id + '-' + BLOCKCHAIN.network + '">https://dpos.space/golos/escrow/?id=' + from + '-' + escrow_id + '-' + BLOCKCHAIN.network + '</a>');
	...
	golos.api.getContentReplies(BLOCKCHAIN.agents_list_post_author, BLOCKCHAIN.agents_list_post_permlink, function(err, result) {
		if(!err) {
			result.sort(function(a, b) {
				if (parseInt(a.author_reputation) < parseInt(b.author_reputation))
					return 1;
	...
	golos.api.getDynamicGlobalProperties(function(err, response) {
		...
		$.ajax({
			dataType: "json",
			url: '/blockchains/golos/apps/escrow/l10n/' + currentLanguage + '.json?1',
			success: function(r) {
				LNG = r;
				changeLanguage();
			},
