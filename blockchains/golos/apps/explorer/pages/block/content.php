...
$benif = $key.': [';
  ...
  $benif .= '{
account: "<a href="'.$site_url.'golos/profiles/'.$benefactor['account'].'" target="_blank">'.$benefactor['account'].'</a>,
...
$content = '<h2>Блок №'.$datas.' (<a href="'.$conf['siteUrl'].'golos/explorer/block/'.$prev_block.'" target="_blank">← предыдущий</a>, <a href="'.$conf['siteUrl'].'golos/explorer/block/'.$next_block.'" target="_blank">→ следующий</a>)</h2>
<ul><li>Сформирован '.$timestamp.' GMT</li>
<li>Подписал делегат <a href="'.$conf['siteUrl'].'golos/profiles/'.$witness.'/witness" target="_blank">'.$witness.'</a></li></ul>
<hr />
<h3>Транзакции</h3>
<ol>';
$prev_id = '';
...
if ($num !== 0) {
  ...
  $content .= '<li><h4>id: <a href="'.$conf['siteUrl'].'golos/explorer/tx/'.$tr['trx_id'].'" target="_blank">'.$tr['trx_id'].'</a></h4>
