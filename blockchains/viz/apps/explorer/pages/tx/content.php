...
function convert_operation_data($arr, $site_url) {
  ...
  foreach ($arr as $key => $value) {
    if ($key === 'from' || $key === 'initiator' || $key === 'receiver' || $key === 'to' || $key === 'account' || $key === 'account_seller' || $key === 'subaccount_seller' || $key === 'seller' || $key === 'benefactor' || $key === 'new_account_name' || $key === 'witness' || $key === 'voter' || $key === 'delegator' || $key === 'delegatee' || $key === 'creator' || $key === 'subscriber' || $key === '') {
