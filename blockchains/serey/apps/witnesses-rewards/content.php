<?php if (!defined('NOTLOAD')) exit('No direct script access allowed');
  global $conf;
    $html = file_get_contents('http://178.20.43.121:3179/serey-api?service=witnesses');
...
<table id="table"><thead><tr><th>Логин</th><th>за вчерашний день</th><th>за сегодня</th><th>за прошлый месяц</th><th>за текущий месяц</th></tr></thead><tbody id="target">';
    ...
    foreach ($table as $witness) {
      $content .= '<tr align="right"><td><a href="'.$conf['siteUrl'].'serey/profiles/'.$witness['login'].'/witness" target="_blank">'.$witness['login'].'</a></td>
