...
if (accounts && accounts.length > 0) {
    ...
    if (selected === 'SEREY') {
        ...
        $('#max_amount').html(parseFloat(acc.hbd_balance));
                $('#buy_token').html('SEREY');
            }
}
}

$(document).ready(async function() {
    $('#orders_history').attr('href', `https://dpos.space/serey/profiles/${serey_login}/orders`);
...
if ($('#sell_token').val() === 'HBD') {
    history.pushState([], '', `/serey/swap/HBD`);
} else {
    history.pushState([], '', `/serey/swap`);
}
});
