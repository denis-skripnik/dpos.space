...
$url = explode('@', $op['url'])[1];
$url = explode('#', $url)[0];
            $result['content'] .= '<tr>
<td><a href="'.$site_url.'serey/profiles/'.$parent_author.'" target="_blank">'.$parent_author.'</a></td>
<td>'.$comment_created.'</td>
<td>'.$markdown_text.'</td>
<td><a href="https://serey.io/authors/'.$url.'" target="_blank">'.$op['root_title'].'</a></td>
</tr>';
        }
    }
}
