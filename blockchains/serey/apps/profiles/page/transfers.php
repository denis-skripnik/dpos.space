...
if ($op[0] == 'claim') $memo = 'Получение своих начислений на SP.';
...
$result['content'] .= '<tr>
<td>'.$timestamp.'</td>
        <td><a href="'.$site_url.'serey/profiles/'.$creator.'" target="_blank">'.$creator.'</a></td>
<td></td>
<td>'.$balance.'</td>
<td>'.$memo.'</td>
</tr>';
...
$initiator = $op[1]['initiator'];
    ...
    $result['content'] .= '<tr>
    <td>'.$timestamp.'</td>
            <td><a href="'.$site_url.'serey/profiles/'.$initiator.'" target="_blank">'.$initiator.'</a></td>
    <td><a href="'.$site_url.'serey/profiles/'.$receiver.'" target="_blank">'.$receiver.'</a></td>
    <td></td>
    <td>'.$memo.'</td>
    </tr>';
                    if ($rowCount === TRX_LIMIT) {
