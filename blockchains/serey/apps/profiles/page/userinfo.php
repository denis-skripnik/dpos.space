...
if( isset(pageUrl()[2]) && isset($mass) && count($mass) > 0){ // проверяем существование элемента
 ...
 $HP = (float)$mass[0]['vesting_shares'];
$delegated_HP = (float)$mass[0]['received_vesting_shares'];
$delegating_HP = (float)$mass[0]['delegated_vesting_shares'];
$vesting_withdraw_rate = (float)$mass[0]['vesting_withdraw_rate'];
...
<tr><td>Личная SP</td>
<td>".round($HP, 3)." SEREY</td>
</tr>";
if ($delegated_HP > 0) {
$content .= '<tr><td>Получено в пользование  Power</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/serey/apps/profiles/page/delegations.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=received">'.round($delegated_HP, 3).'</a> SEREY</td>
</tr>';
}
if ($vesting_withdraw_rate > 0) {
$content .=  "<tr><td>Сумма вывода из  Power</td>
<td>".round($vesting_withdraw_rate, 3)." SEREY</td>
</tr>
...
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/serey/apps/profiles/page/delegations.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=delegated">'.round($delegating_HP, 3).'</a> SEREY</td></tr>';
}
if ($datas['proxied_vsf_votes'][0] != 0) {
    ...
    <td>".round($all_shares, 3)." SEREY</td></tr>";
    $full_HP = $tvsh;
...
if (isset($json_metadata['profile']['select_tags'])) {
 ...
 $content .= '<tr><td>Баланс SEREY</td>
<td>'.$chain_balance.'</td>
</tr>
</tr>';
if ($savings_balance !== '0.000 SEREY') {
$content .= "<tr><td>SEREY в сейфе</td>
...
$content .=  '<tr>
<td>ов за делегатов</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/serey/apps/profiles/page/delegat.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'">'.$datas['witnesses_voted_for'].'</a></td></tr>
</table>
...
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/serey/apps/profiles/page/followers.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=followers">'.$follower_count.'</a></td></tr>
<tr><td>Подписался на блогов</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/serey/apps/profiles/page/followers.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=followings">'.$following_count.'</a></td></tr>
<tr>
<td>Аккаунт-регистратор (С его помощью можно восстановить ваш аккаунт, он вас зарегистрировал в блокчейне)</td>
<td><a href="'.$conf['siteUrl'].'serey/profiles/'.$recovery_account.'" target="_blank">'.$recovery_account.'</a></td>
</tr>';
