...
$newStartPermlink = $start_permlink;
                break;
                  ...
                  $result['content'] .= '<tr>
                  <td>'.$created.'</td>
                  <td><a href="https://serey.io/authors/' . $content['author'] . '/' . $content['permlink'] . '" target="_blank">' . $content['title'] . '</a></td>
<td><a href="'.$site_url.'serey/profiles/' . $content['author'] . '" target="_blank">' . $content['author'] . '</a></td>
<td>';
if (isset($metadata['tags'])) {
    $tags_str = '';
    foreach ($tegi as $teg) {
