...
function fillContent(err, result) {
 ...
 $('#content_payouts').val(result.percent_hive_dollars);
...
$("#load4edit").click(function() {
     ...
     hive.api.getContent(Author, Permlink, fillContent)
 
});

hive.api.getChainProperties(function(err, res) {
if (!err) {
...
var percent_hive_dollars = document.getElementById('content_payouts').value;
 ...
 if (isEdit === true) {
         ...
         const operations = [
             ...
             hive.broadcast.send({extensions: [], operations}, [wif], function(err, res) {
                 if(err) {
                     if(err == 'RPCError: Assert Exception:( now - auth.last_root_post ) > SEREY_MIN_ROOT_COMMENT_INTERVAL: You may only post once every 5 minutes.') {
...
let operations = [
 ['comment', {'parent_author':'','parent_permlink':parentPermlink,'author':serey_login,'permlink':permlink,'title':title,'body':body,'json_metadata':JSON.stringify(jsonMetadata)}],['comment_options',{'author':serey_login,'permlink':permlink,'max_accepted_payout':'1000000.000 HBD','percent_hbd':+percent_hive_dollars,'allow_votes':true,'allow_curation_rewards':true,extensions}]];
     hive.broadcast.send({extensions: [], operations}, [wif], function(err, res) {
         if(err) {
             if(err == 'RPCError: Assert Exception:( now - auth.last_root_post ) > SEREY_MIN_ROOT_COMMENT_INTERVAL: You may only post once every 5 minutes.') {
             alert('Подождите хотя-бы 5 минут перед публикацией следующего поста!');
           ...
           alert('Пост опубликован успешно! \n URL поста: \n@' + serey_login + '/' + permlink);
         }
     })    
}
}
hasPost(serey_login, permlink, postSender);
}
