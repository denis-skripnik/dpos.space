function checkWorkingNode() {
    ...
    let node = localStorage.getItem("serey_node") || NODES[0];
        ...
        steem.api.getDynamicGlobalPropertiesAsync()
            .then(props => {
                if(!timeout) {
                    ...
                    localStorage.setItem("serey_node", node);
                    clearTimeout(timer);
                }
            })
            .catch(e => {
...
let current_user = JSON.parse(localStorage.getItem("serey_current_user"));
if (current_user) {
var serey_login = current_user.login;
var posting_key = sjcl.decrypt('dpos.space_serey_' + serey_login + '_postingKey', current_user.posting);
var active_key = sjcl.decrypt('dpos.space_serey_' + serey_login + '_activeKey', current_user.active);
...
var users = JSON.parse(localStorage.getItem('serey_users'));
$( document ).ready(function() {
...
function selectAccount() {
    let current_user = JSON.parse(localStorage.getItem("serey_current_user"));
    users = JSON.parse(localStorage.getItem('serey_users'));
    if (users) {
    let radioButtons = '';
    if (users.length === 1) {
...
function deleteAccount(login) {
    ...
    localStorage.setItem("serey_users", JSON.stringify(new_list));
    selectAccount()
    $('#delete_msg').html('Аккаунт ' + login + ' удалён из списка.');
    } else if (users.length === 1) {
        selectAccount()
            $('#delete_msg').html('Аккаунт ' + login + ' удалён из списка.');
            localStorage.removeItem('serey_users');
            localStorage.removeItem('serey_current_user');
        }
}
