function checkWorkingNode() {
    ...
    let node = localStorage.getItem("steem_node") || NODES[0];
        ...
        steem.api.getDynamicGlobalPropertiesAsync()
            .then(props => {
                if(!timeout) {
                    ...
                    localStorage.setItem("steem_node", node);
                    clearTimeout(timer);
                }
            })
            .catch(e => {
...
let current_user = JSON.parse(localStorage.getItem("steem_current_user"));
if (current_user) {
var steem_login = current_user.login;
var posting_key = sjcl.decrypt('dpos.space_steem_' + steem_login + '_postingKey', current_user.posting);
var active_key = sjcl.decrypt('dpos.space_steem_' + steem_login + '_activeKey', current_user.active);
...
var users = JSON.parse(localStorage.getItem('steem_users'));
$( document ).ready(function() {
...
function selectAccount() {
    let current_user = JSON.parse(localStorage.getItem("steem_current_user"));
    users = JSON.parse(localStorage.getItem('steem_users'));
    if (users) {
    let radioButtons = '';
    if (users.length === 1) {
...
function deleteAccount(login) {
    ...
    localStorage.setItem("steem_users", JSON.stringify(new_list));
    selectAccount()
    $('#delete_msg').html('Аккаунт ' + login + ' удалён из списка.');
        ...
        selectAccount()
            ...
            localStorage.removeItem('steem_current_user');
        }
}
