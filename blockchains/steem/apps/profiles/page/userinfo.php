...
$steem_i = (float)$rewa / (float)$recent;
$dasdas_steem = round($steem_r * $steem_m * 100 * $steem_i, 3);
$dasdas_sbd = round($steem_r * $steem_m * 100 * $steem_i * $ticker_price, 3);
$fixx_steem = round($steem_r * $fixx_steem_m * 100 * $steem_i, 3);
$fixx_sbd = round($steem_r * $fixx_steem_m * 100 * $steem_i * $ticker_price, 3);
...
<tr><td>Личная SP</td>
<td>".round($sp, 3)." STEEM</td>
</tr>";
if ($delegated_sp > 0) {
$content .= '<tr><td>Получено в пользование Steem Power</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/steem/apps/profiles/page/delegations.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=received">'.round($delegated_sp, 3).'</a> STEEM</td>
</tr>';
}
if ($vesting_withdraw_rate > 0) {
$content .=  "<tr><td>Сумма вывода из Steem Power</td>
<td>".round($vesting_withdraw_rate, 3)." STEEM</td>
</tr>
<tr><td>Следующий вывод</td>
<td>$next_vesting_withdrawal</td></tr>";
}
if ($delegating_sp > 0) {
$content .=  '<tr><td>Делегировано SP другим аккаунтам</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/steem/apps/profiles/page/delegations.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=delegated">'.round($delegating_sp, 3).'</a> STEEM</td></tr>';
}
if ($datas['proxied_vsf_votes'][0] != 0) {
    $content .=  "<tr><td>Прокси SP для голосования за делегатов</td>
    <td>$proxy_shares Ƶ</td></tr>";
}
    $content .=  "<tr><td>Итоговое количество SP, влияющее на силу апвота</td>
    <td>".round($all_shares, 3)." STEEM</td></tr>";
    if ($dasdas_sbd == 0) {
...
if (isset($json_metadata['profile']['select_tags'])) {
 ...
 $content .= '<tr><td>Баланс STEEM</td>
<td>'.$chain_balance.'</td>
</tr>
<tr><td>Баланс SBD</td>
<td>'.$sbd_balance.'</td>
</tr>';
if ($savings_balance !== '0.000 STEEM') {
$content .= "<tr><td>STEEM в сейфе</td>
<td>$savings_balance</td>
</tr>";
}
if ($savings_sbd_balance !== '0.000 SBD') {
$content .= "<tr><td>SBD в сейфе</td>
...
$content .=  '<tr>
<td>Steemов за делегатов</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/steem/apps/profiles/page/delegat.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'">'.$datas['witnesses_voted_for'].'</a></td></tr>
</table>
...
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/steem/apps/profiles/page/followers.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=followers">'.$follower_count.'</a></td></tr>
<tr><td>Подписался на блогов</td>
<td><a data-fancybox class="ajax_modal" data-src="#ajax_modal_content" href="javascript:;" data-url="'.$conf['siteUrl'].'blockchains/steem/apps/profiles/page/followers.php" data-params="user='.$user.'&siteUrl='.$conf['siteUrl'].'&type=followings">'.$following_count.'</a></td></tr>
<tr>
<td>Аккаунт-регистратор (С его помощью можно восстановить ваш аккаунт, он вас зарегистрировал в блокчейне)</td>
<td><a href="'.$conf['siteUrl'].'steem/profiles/'.$recovery_account.'" target="_blank">'.$recovery_account.'</a></td>
</tr>';
