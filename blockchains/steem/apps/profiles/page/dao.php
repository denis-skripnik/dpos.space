...
while ($rowCount !== TRX_LIMIT && $retry_counter < 3) {
    ...
    foreach ($mass as $datas) {
        ...
        $rowCount++;
        $name = 'Steemование по заявке воркера';
...
<td>Заявка <a href="'.$site_url.'steem/manage/workers#'.$author.'/'.$permlink.'" target="_blank">'.$author.'/'.$permlink.'</a></td>
...
<td><a href="'.$site_url.'steem/profiles/'.$witness.'" target="_blank">'.$witness.'</a></td>
<td>'.$name.' '.$approve.'</td>
</tr>';
                if ($rowCount === TRX_LIMIT) {
            break;
...
<td>'.$timestamp.'</td>
<td><a href="'.$site_url.'steem/profiles/'.$account.'" target="_blank">'.$account.'</a></td>
<td><a href="'.$site_url.'steem/profiles/'.$proxy.'" target="_blank">'.$proxy.'</a></td>
<td>'.$name.'</td>        
</tr>';
                if ($rowCount === TRX_LIMIT) {
            break;
...
$result['content'] .= '<tr>
<td>'.$timestamp.'</td>
<td><a href="'.$site_url.'steem/profiles/'.$author.'" target="_blank">'.$author.'</a></td>
<td><a href="'.$site_url.'steem/profiles/'.$worker.'" target="_blank">'.$worker.'</a></td>
<td>'.$name.'. '.$amounts.', в Steem Power: '.$vest_reward.' <a href="https://steemit.com/@'.$author.'/'.$permlink.'" target="_blank">Пост</a></td>
</tr>';
                if ($rowCount === TRX_LIMIT) {
            break;
        }
