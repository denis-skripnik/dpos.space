...
if (!isset($user) && isset($_REQUEST['options']['user'])) { // проверяем существование элемента
    ...
    $result['content'] = '<div id="transfers_content"><h2>Steem Power пользователя '.$user.'</h2>
<table>
<tr>
<th>Дата и время</th>
<th>От кого</th>
...
while ($startWith !== -1 && $rowCount !== TRX_LIMIT) {
    ...
    foreach ($mass as $datas) {
        ...
        $amount = round($float_amount, 3).' STEEM';
        $memo = '';
...
$result['content'] .= '<tr>
<td>'.$timestamp.'</td>
		<td><a href="'.$site_url.'steem/profiles/'.$from.'" target="_blank">'.$from.'</a></td>
<td><a href="'.$site_url.'steem/profiles/'.$to.'" target="_blank">'.$to.'</a></td>
<td>'.$amount.'</td>
<td>'.$memo.'</td>
</tr>';
        ...
        $amount = round($float_amount, 3).' STEEM';
        $memo = 'Запуск вывода из Steem Power.';
        if ($float_amount == 0) $memo = 'Отмена вывода SP';
        $month = array('01' => 'января', '02' => 'февраля', '03' => 'марта', '04' => 'апреля', '05' => 'мая', '06' => 'июня', '07' => 'июля', '08' => 'августа', '09' => 'сентября', '10' => 'октября', '11' => 'ноября', '12' => 'декабря');
        $timestamp1 = $datas[1]['timestamp'];
 $timestamp2 = strtotime($timestamp1);
$month2 = date('m', $timestamp2);
...
$result['content'] .= '<tr>
<td>'.$timestamp.'</td>
		<td colspan="2"><a href="'.$site_url.'steem/profiles/'.$from.'" target="_blank">'.$from.'</a></td>
<td>'.$amount.'</td>
<td>'.$memo.'</td>
</tr>';
                if ($rowCount === TRX_LIMIT) {
