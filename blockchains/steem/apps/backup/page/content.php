...
<li>Отправьте 1 STEEM или 0.5 SBD на аккаунт @'.$amount_account.'. Заметка (memo):<br />
posts;<br></li>
...
<p><label for="WLS_login">Имя пользователя (логин) в Steem (Без "@"):</label>
...
$op = $datas[1]['op'];
$tokens3 = "0.500 SBD";
if ($op['0'] === 'transfer' && $op['1']['from'] === pageUrl()[2] && $op['1']['to'] === $amount_account && (($op['1']['amount'] ?? $op['1']['amount'] ?? "") === $tokens3 or ($op['1']['amount'] ?? $op['1']['amount'] ?? "") === "1.000 STEEM") && $op['1']['memo'] === "posts") {
if (isset($op) ){
...
if (!isset($op_trx) ){
$content .= '<p>Пользователь <a href="https://steemitwallet.com/@'.pageUrl()[2].'/" target="_blank">@'.pageUrl()[2].'</a> не произвёл платёж<br />
