...
function getBestDay($acc, $curr) {
    return ((float)$acc['invest_days'] > (float)$curr['invest_days'] ? $acc : $curr);
}
...
$best_invest_day = (float)$best_invest_day['invest_days'];
...
foreach($res['providers'] as $key => $provider) {
  $key++;
  $share = (float)$provider['liquidity'] / $l;
$bip_liquidity = $all_bip_liquidity * $share;
[$days, $supers_counter, $super] = getCalcData($key, $provider, $percent);

$provider_percent = $percent + (($max_percent - $percent) * ((float)$days / (float)$best_invest_day));
