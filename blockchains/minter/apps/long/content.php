...
$farming_amount = $max_amount - ($max_prize * 2);
...
function allExperienceSum($a, $b) {
  $a += (float)$b['liquidity'] * ((1 + (float)$b['invest_days'] / 100));
  return $a;
}
...
<strong>Инвест. дни и отправка производится от суммы в 0.2 LONG (это сделано, чтоб сумма отправки не была меньше комиссии), а также только если количество получений <= 500.</strong></p>
...
<li>Максимальная сумма в лото и бонусах за инвест. дни (за день): '.$max_prize.' LONG</li>
...
foreach($res['providers'] as $key => $provider) {
  ...
  $experience = (float)$provider['liquidity'] * (1 + ((float)$provider['invest_days'] / 100));
