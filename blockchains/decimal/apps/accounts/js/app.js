...
return;
    ...
    let crypt_seed = sjcl.encrypt('dpos.space_decimal_' + login + '_seed', seed);

if (!users) {
users = [];
}
if (!localStorage.getItem('decimal_users')|| localStorage.getItem('decimal_users').indexOf(login) === -1) {
if (seed) {
    let acc_data = {login, seed: crypt_seed};
    localStorage.setItem("decimal_current_user", JSON.stringify(acc_data));
    users.push(acc_data);
    localStorage.setItem("decimal_users", JSON.stringify(users));
