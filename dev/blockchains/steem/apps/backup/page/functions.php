...
function archive($app_dir) {
	...
	<form><input type="url" name="download_url" value="'.$conf['siteUrl'].'blockchains/steem/apps/backup/archives/'.pageUrl()[2].'.zip"></form></p>';
	} else {
...
function html_archive($app_dir) {
	...
	<p>Либо вы можете скопировать адрес из поля ниже:
	<form><input type="url" name="download_url" value="'.$conf['siteUrl'].'steem/backup/archives/'.'html_'.pageUrl()[2].'.zip"></form></p>';
	} else {
	die ('Произошла ошибка при создании архива');
	}
return $content;
