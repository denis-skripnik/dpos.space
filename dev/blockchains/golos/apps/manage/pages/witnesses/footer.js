...
$('#witnesses_list').append(`<li><label><input type="checkbox" checked name="witnesses" value="${witness.owner}">${status} делегат <a href="https://dpos.space/dev/golos/profiles/${witness.owner}" target="_blank">${witness.owner}</a>, <a href="${witness.url}" target="_blank">${url_text}</a>, <a href="https://dpos.space/dev/golos/profiles/${witness.owner}/witness" target="_blank">Параметры</a></label></li>`);
} else {
...
$('#witnesses_list').append(`<li><label><input type="checkbox" name="witnesses" value="${witness.owner}">${status} делегат <a href="https://dpos.space/dev/golos/profiles/${witness.owner}" target="_blank">${witness.owner}</a>, <a href="${witness.url}" target="_blank">${url_text}</a></label></li>`);
end_witness = witness.owner;
}
}
...
golos.api.getAccounts([golos_login], function(err, res) {
    if (res[0].proxy !== '') {
        $('#proxy').html('<p>Аккаунт <a href="https://dpos.space/dev/golos/profiles/' + golos_login + '" target="_blank">' + golos_login + '</a> установил в качестве прокси <a href="https://dpos.space/dev/golos/profiles/' + res[0].proxy + '" target="_blank">' + res[0].proxy + '</a></p>');
