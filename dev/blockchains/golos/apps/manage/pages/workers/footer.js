...
function allworkerVotes(author, permlink, start_voter) {
    golos.api.getWorkerRequestVotes(author, permlink, start_voter, 50, function(err, result) {
      ...
      if (start_voter !== '' && vote > 0) {
                $('#request_votes').append(`<li><a href="https://dpos.space/dev/golos/profiles/${result[vote].voter}" target="_blank">@${result[vote].voter}</a> проголосовал на ${result[vote].vote_percent / 100}%</li>`);
...
$('#request_creator').html(`<a href="https://dpos.space/dev/golos/profiles/${result.post.author}" target="_blank">${result.post.author}</a>`);
$('#request_worker').html(`<a href="https://dpos.space/dev/golos/profiles/${result.worker}" target="_blank">${result.worker}</a>`);
...
<td><a href="https://dpos.space/dev/golos/profiles/${request.post.author}" target="_blank">${request.post.author}</a></td>
<td><a href="https://dpos.space/dev/viz/profiles/${request.worker}" target="_blank">${request.worker}</a></td>
