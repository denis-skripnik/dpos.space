...
body_received_vesting_shares = '<tr><td><a href="/dev/golos/profiles/' + item.delegator + '" target="_blank">@' + item.delegator + '</a></td><td>' + vs_amount + '</td><td>' + interest_rate + '%</td><td>' + min_delegation_datetime + '</td></tr>';
...
let min_delegation_time = Date.parse(item.min_delegation_time);
let min_delegation_datetime = date_str(min_delegation_time - timezoneOffset, true, false, true);
body_delegated_vesting_shares = '<tr id="delegated_vesting_shares_' + item.delegatee + '"><td><a href="/dev/golos/profiles/' + item.delegatee + '" target="_blank">@' + item.delegatee + '</a></td><td>' + vesting_shares_amount + '</td><td>' + interest_rate + '%</td><td>' +  min_delegation_datetime + '</td><td><input type="button" id="cancel_delegated_vesting_shares_' + item.delegatee + '" value="Отменить делегирование"></td></tr>';
...
function prepareContent(text) {
  ...
  }).replace(/ (@[^< \.,]+)/gi, user => ` <a href="/dev/golos/profiles/${user.trim().slice(1)}">${user.trim()}</a>`)
 }
...
<td><a href="/dev/golos/profiles/' + from + '" target="_blank">@' + from + '</a></td>\
<td><a href="/dev/golos/profiles/' + to + '" target="_blank">@' + to + '</a></td>\
<td>' + amount + '</td>\
<td>' + memo + '</td>\
</tr>');
...
<td><a href="/dev/golos/profiles/' + author + '" target="_blank">@' + author + '</a></td>\
<td><a href="/dev/golos/profiles/' + curator + '" target="_blank">@' + curator + '</a></td>\
<td>' + reward + '</td>\
<td>' + memo + '</td>\
</tr>');
...
<td><a href="/dev/golos/profiles/' + author + '" target="_blank">@' + author + '</a></td>\
<td><a href="/dev/golos/profiles/' + benefactor + '" target="_blank">@' + benefactor + '</a></td>\
...
<td>' + from + '</td>\
<td><a href="/dev/golos/profiles/' + witness + '" target="_blank">@' + witness + '</a></td>\
<td>' + shares + '</td>\
<td>' + memo + '</td>\
</tr>');
  }
