...
let sp = parseFloat(acc.vesting_shares) / 1000000 * hive_per_vests;
sp = sp.toFixed(6);
sp = parseFloat(sp);
let received_sp = parseFloat(acc.received_vesting_shares) / 1000000 * hive_per_vests;
received_sp = received_sp.toFixed(6);
received_sp = parseFloat(received_sp);
let delegated_sp = parseFloat(acc.delegated_vesting_shares) / 1000000 * hive_per_vests;
...
vesting_shares_amount = parseFloat(item.vesting_shares) / 1000000 * hive_per_vests;
...
body_delegated_vesting_shares = '<tr id="delegated_vesting_shares_' + item.delegatee + '"><td><a href="/dev/hive/profiles/' + item.delegatee + '" target="_blank">@' + item.delegatee + '</a></td><td>' + vesting_shares_amount + '</td><td>' +  min_delegation_datetime + '</td><td><input type="button" id="cancel_delegated_vesting_shares_' + item.delegatee + '" value="Отменить делегирование"></td></tr>';
...
hive.broadcast.delegateVestingShares(active_key, account, item.delegatee, '0.000000 VESTS', function(err, result) {
if (!err) {
...
var vesting_withdraw_rate = parseFloat(acc.vesting_withdraw_rate) / 1000000 * hive_per_vests;
...
function prepareContent(text) {
  ...
  }).replace(/ (@[^< \.,]+)/gi, user => ` <a href="/dev/hive/profiles/${user.trim().slice(1)}">${user.trim()}</a>`)
 }
...
async function walletData() {
  ...
  while (! isCompleted && ! isEnd) {
    ...
    const data = await hive.api.getAccountHistoryAsync(hive_login, from, limitReal);
...
items.forEach(item => {
  ...
  if (op[1].from && op[1].to && op[1].amount) {
    ...
    if (op[1].memo) {
      ...
      var memo = 'Перевод в HP.';
    } else {
      var memo = '';
    }
    jQuery("#transfer_history_tbody").append('<tr class="filtered ' + from + '"><td>' + transfer_datetime + '</td>\
<td><a href="/dev/hive/profiles/' + from + '" target="_blank">@' + from + '</a></td>\
<td><a href="/dev/hive/profiles/' + to + '" target="_blank">@' + to + '</a></td>\
<td>' + amount + '</td>\
<td>' + memo + '</td>\
</tr>');
...
<td><a href="/dev/hive/profiles/' + author + '" target="_blank">@' + author + '</a></td>\
<td><a href="/dev/hive/profiles/' + curator + '" target="_blank">@' + curator + '</a></td>\
<td>' + reward + '</td>\
<td>' + memo + '</td>\
</tr>');
...
<td><a href="/dev/hive/profiles/' + author + '" target="_blank">@' + author + '</a></td>\
<td><a href="/dev/hive/profiles/' + benefactor + '" target="_blank">@' + benefactor + '</a></td>\
<td>' + sbd_payout + ', ' + hive_payout + ' и ' + vesting_payout + '</td>\
<td>' + memo + '</td>\
</tr>');
    ...
    var shares = parseFloat(op[1].vesting_shares) / 1000000 * hive_per_vests;
    shares = shares.toFixed(6) + ' HP';
    var memo = 'Награда делегата.';
    jQuery("#transfer_history_tbody").append('<tr class="filtered_witness_reward"><td>' + transfer_datetime + '</td>\
<td>' + from + '</td>\
<td><a href="/dev/hive/profiles/' + witness + '" target="_blank">@' + witness + '</a></td>\
<td>' + shares + '</td>\
<td>' + memo + '</td>\
</tr>');
  }
