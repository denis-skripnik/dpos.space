...
body_received_vesting_shares = '<tr><td><a href="/dev/viz/profiles/' + item.delegator + '" target="_blank">@' + item.delegator + '</a></td><td>' + vs_amount + ' Ƶ</td><td>' + min_delegation_datetime + '</td></tr>';
...
let min_delegation_time = Date.parse(item.min_delegation_time);
let min_delegation_datetime = date_str(min_delegation_time - timezoneOffset, true, false, true);
body_delegated_vesting_shares = '<tr id="delegated_vesting_shares_' + item.delegatee + '"><td><a href="/dev/viz/profiles/' + item.delegatee + '" target="_blank">@' + item.delegatee + '</a></td><td>' + vesting_shares_amount + ' Ƶ</td><td>' + min_delegation_datetime + '</td><td><input type="button" id="cancel_delegated_vesting_shares_' + item.delegatee + '" value="Отменить делегирование"></td></tr>';
...
function prepareContent(text) {
  ...
  }).replace(/ (@[^< \.,]+)/gi, user => ` <a href="/dev/viz/profiles/${user.trim().slice(1)}">${user.trim()}</a>`)
 }
...
<td><a href="/dev/viz/profiles/' + from + '" target="_blank">@' + from + '</a></td>\
<td><a href="/dev/viz/profiles/' + to + '" target="_blank">@' + to + '</a></td>\
<td>' + amount + '</td>\
<td>' + memo + '</td>\
  </tr>');
    } else if (op[0] === 'award') {
...
<td><a href="/dev/viz/profiles/' + initiator + '" target="_blank">@' + initiator + '</a></td>\
<td><a href="/dev/viz/profiles/' + receiver + '" target="_blank">@' + receiver + '</a></td>\
<td></td>\
<td>' + memo + '</td>\
  </tr>');
  ...
  <td>' + memo + '</td>\
      ...
      var receiver = op[1].receiver;
      var amount = op[1].shares;
      var memo = `Вы получили бенефициарские. Получатель награды: <a href="/dev/viz/profiles/${receiver}" target="_blank">@${receiver}</a>. Заметка: ${op[1].memo}`;
      jQuery("#transfer_history_tbody").append('<tr class="filtered_benefactor_award"><td>' + transfer_datetime + '</td>\
  <td><a href="/dev/viz/profiles/' + from + '" target="_blank">@' + from + '</a></td>\
  <td><a href="/dev/viz/profiles/' + benefactor + '" target="_blank">@' + benefactor + '</a></td>\
  <td>' + amount + '</td>\
  <td>' + memo + '</td>\
	</tr>');
  ...
  <td>' + from + '</td>\
  <td><a href="/dev/viz/profiles/' + witness + '" target="_blank">@' + witness + '</a></td>\
  <td>' + shares + '</td>\
  <td>' + memo + '</td>\
	</tr>');
    }
