...
$('#request_creator').html(`<a href="https://dpos.space/dev/viz/profiles/${result.creator}" target="_blank">${result.creator}</a>`);
$('#request_worker').html(`<a href="https://dpos.space/dev/viz/profiles/${result.worker}" target="_blank">${result.worker}</a>`);
...
for (let vote of votes) {
    ...
    $('#request_votes').append(`<li>${vote_datetime} <a href="https://dpos.space/dev/viz/profiles/${vote.voter}" target="_blank">@${vote.voter}</a> проголосовал на ${vote.vote_percent / 100}%</li>`);
}
}
    });
}
