...
# JavaScript syntax checks (node --check accepts one script path at a time)
for file in v3/js/*.js tests/*.js; do node --check "$file"; done
...
- Use the configured `origin`; do not rewrite Git remote transport during unrelated work (the current checkout uses HTTPS).
