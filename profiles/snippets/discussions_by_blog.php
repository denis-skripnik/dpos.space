...
class DiscussionsByBlog
{
    ...
    static public function get($user, $chain, $limit, $startAuthor = null, $startPermlink = null)
    {
        $createdMaxDatetime = static::getCreatedMaxDatetime($chain);
        ...
        while (count($posts) !== $limit && is_null($newStartAuthor)) {
            ...
            $chunk = static::getChunk($user, $chain, static::LIMIT_MAX, $startAuthor, $startPermlink);
            $result = $chunk['result'];
    ...
    static private function getCreatedMaxDatetime($chain)
    {
        $paymentDays = $chain === 'WLS' ? 2 : 7;
    ...
    static private function getChunk($user, $chain, $limit, $startAuthor = null, $startPermlink = null)
    {
        $data = [['limit' => $limit]];

        if ($chain == 'golos' or $chain == 'viz') {
            $data[0]['select_authors'] = [$user];
        } else {
            $data[0]['tag'] = $user;
        }
