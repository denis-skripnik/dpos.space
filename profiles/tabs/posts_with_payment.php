...
define('POSTS_WITH_PAYMENT_LIMIT', 50);

require_once $_SERVER['DOCUMENT_ROOT'] . '/vendor/autoload.php';
require $_SERVER['DOCUMENT_ROOT'].'/profiles/snippets/discussions_by_blog.php';
...
$result = [];
$result['content'] = "<h2>Посты, получившие выплаты</h2>
...
$res = $isNotFirstPage ? DiscussionsByBlog::get($user, $chain, POSTS_WITH_PAYMENT_LIMIT, $startAuthor, $startPermlink) :
    DiscussionsByBlog::get($user, $chain, POSTS_WITH_PAYMENT_LIMIT);
...
foreach ($posts as $post) {
    ...
    if ($post['author'] == $user) {
        $result['content'] .= '<td><a href="https://' . $client . $post['url'] . '" target="_blank">' . $post['title'] . '</a></td>';
    } else {
        $result['content'] .= '<td><a href="https://' . $client . $post['url'] . '" target="_blank">' . $post['title'] . '</a> (Репост от <a href="https://dpos.space/profiles/' . $post['author'] . '/' . $chain . '" target="_blank">@' . $post['author'] . '</a>)</td>';
    }

    $createdTime = strtotime($post['created']);
    $cashoutTime = $createdTime + ($chain === 'WLS' ? 2 : 7) * 24 * 60 * 60;
...
$round_days = $chain === 'WLS' ? 'двух дней' : 'семи дней';
