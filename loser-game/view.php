<?php
$html = file_get_contents('http://94.130.180.46:3000/loser-game?chain='.$array_url[1].'&type=active');
    $table = json_decode($html, true);
 echo '<h2>Создайте свой раунд</h2>
...
if ($table) {
 foreach ($table as $round) {
    echo '<tr><td>'.$round['amount'].' '.$round['token'].'</td>';
$transfers = '';
    foreach($round['transfers'] as $transfer) {
$transfers .= '<a href="https://'.$client.'/@'.$transfer.'" target="_blank">@'.$transfer.'</a>, ';
}
$transfers = substr($transfers,0,-2);
echo '<td>'.$transfers.'</td>
...
function submitJoin(sender, clients, amount, token) {
    let data_url = [];
    data_url.push(["transfer",{"from":sender,"to":"loser","amount":amount + " " + token,"memo":"Играю при помощи https://dpos.space/loser-game/'.$array_url[1].'!"}]);
    ...
    ;    if (clients === "sign") {
        ...
        window.open("https://golos.id/@" + sender + "/transfers?to=loser&amount=" + amount + "&token=" + token + "&memo=Играю при помощи https://dpos.space/loser-game/'.$array_url[1].'!");
        location.reload();
    }
}
