...
Ветка `v3` содержит статическую HTML+JS-версию без обязательного PHP/backend runtime. Запускайте её через локальный HTTP-сервер из корня репозитория, а не через `file://`.
...
Если порт `8080` занят, укажите другой порт и откройте соответствующий URL. Legacy PHP-файлы остаются в репозитории, но для v3 достаточно статической раздачи файлов.
